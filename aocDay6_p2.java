...
public class aocDay6_p2 {

    public static boolean checkMap(Vector<StringBuilder> map, int posI, int posJ){
        String dir = "up";
        int res = 0;
        int ogPosI = posI;
        int ogPosJ = posJ;
        ...
        while (posI >= 0 && posI <= map.size() && posJ >= 0 && posJ <= map.get(0).length()-1){
            ...
            moves++;
            if(moves > (map.size() * map.get(0).length()))
                return true;
        }
        return false;
    }
